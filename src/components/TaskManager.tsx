...
const categoryConfig = {
  ...
  study: { label: 'مطالعه', icon: '📚', color: 'text-primary bg-primary/10 border-primary/20' },
  health: { label: 'سلامت', icon: '💪', color: 'text-success bg-success/10 border-success/20' },
  personal: { label: 'شخصی', icon: '👤', color: 'text-accent bg-accent/10 border-accent/20' },
  project: { label: 'پروژه', icon: '🚀', color: 'text-warning bg-warning/10 border-warning/20' },
};

const priorityConfig = {
  high: { label: 'بالا', icon: '🔴', color: 'text-destructive bg-destructive/10 border-destructive/20', xp: 30 },
  medium: { label: 'متوسط', icon: '🟡', color: 'text-warning bg-warning/10 border-warning/20', xp: 20 },
  low: { label: 'پایین', icon: '🟢', color: 'text-success bg-success/10 border-success/20', xp: 10 },
};

export default function TaskManager() {
  const { state, addTask, completeTask, dispatch } = useApp();
  const { tasks } = state;
                ...
                <div>
                  ...
                  {subtasks.length > 0 && (
                    <div className="space-y-2 mt-2">
                      {subtasks.map((st) => (
                        <div key={st.id} className="flex items-center gap-2 p-2 bg-muted rounded-lg">
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 space-y-3">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {task.deadline && (
                                <Badge variant="outline" className={daysLeft !== null && daysLeft < 0 ? 'text-destructive bg-destructive/10' : ''}>
                                  ...
                                  {task.subtasks.map((st) => (
                                    <div key={st.id} className="flex items-center gap-2 text-sm">
                                      <Checkbox
                                        ...
                                        onCheckedChange={() => handleToggleSubtask(task.id, st.id)}
